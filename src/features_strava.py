...
def generate_athlete_stats(df_clean, df_best_efforts, manual_stats_path):
    ...
    df_athletes_summary = pd.DataFrame(columns=[
    ...
    for sport in ['Run', 'Ride', 'Swim', 'Workout']:
        df_athletes_summary[f'Nb_activities_{sport.lower()}'] = (
            df_clean[df_clean['sport_type'] == sport].groupby('athlete_id')['activity_id'].nunique()
        )

    df_athletes_summary['athlete_id'] = df_clean['athlete_id'].unique()
    ...
    for k, v in distances.items():
        df_athletes_summary[k] = df_athletes_summary['athlete_id'].map(v)
    ...
    df_athletes_summary['FC Max'] = df_athletes_summary['athlete_id'].map(fc_max)

    # Best efforts
    for dist_km in [5, 10, 21.1, 42.2]:
        col = f"{int(dist_km)}km PB"
        df_athletes_summary[col] = df_athletes_summary['athlete_id'].map(extract_best_effort_time(df_best_efforts, dist_km))

    # Merge manual stats
    if Path(manual_stats_path).exists():
        manual_stats = pd.read_csv(manual_stats_path)
        common_cols = [c for c in df_athletes_summary.columns if c in manual_stats.columns]
        df_athletes_summary = pd.concat([df_athletes_summary, manual_stats[common_cols]], ignore_index=True)
        df_athletes_summary.drop_duplicates(subset='athlete_id', keep='last', inplace=True)

    df_athletes_summary.fillna(0, inplace=True)
    df_athletes_summary['Nb_activities'] = df_athletes_summary[[
    ...
    df_athletes_summary['VDOT_max'] = df_athletes_summary[[
        'VDOT_5km', 'VDOT_10km', 'VDOT_21km', 'VDOT_42km'
    ]].max(axis=1)

    return df_athletes_summary

def enrich_activities_with_training_load(df_every_splits, df_athletes_summary):
    """Use df_every_splits for per-activity enrichment"""
    fc_max_dict = df_athletes_summary.set_index("athlete_id")["FC Max"].to_dict()
    df_processed_splits = process_splits(df_every_splits, fc_max_dict)
    df_every_splits = df_every_splits.merge(df_processed_splits, on="activity_id", how="left")
    df_every_splits['cv_speed'].fillna(0, inplace=True)

    df_agg = df_every_splits[['athlete_id', 'activity_id', 'start_date', 'cv_speed',
                       'pct_Z1', 'pct_Z2', 'pct_Z3', 'pct_Z4', 'sport_type']].copy()
    df_agg['distance'] = df_every_splits.groupby('activity_id')['distance'].transform('sum')
    df_agg['moving_time'] = df_every_splits.groupby('activity_id')['moving_time'].transform('sum')
    ...
    return df_every_splits, df_agg, dfs_by_sport
...
if __name__ == "__main__":
    ...
    # === 1. Load input data ===
    df_every_splits = pd.read_csv(DATA_DIR / "df_every_splits.csv", parse_dates=["start_date"])
    ...
    # === 3. Enrich activity data (avec df_every_splits) ===
    df_clean_enriched, df_agg, dfs_by_sport = enrich_activities_with_training_load(df_every_splits, df_athletes_summary)

    # === 4. Save results ===
    df_athletes_summary.to_csv(DATA_DIR / "df_athletes_summary.csv", index=False)
