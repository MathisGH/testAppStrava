...
def generate_athlete_stats(df_final, df_best_efforts, manual_stats_path):
    ...
    for sport in ['Run', 'Ride', 'Swim', 'Workout']:
        df_athletes_stats[f'Nb_activities_{sport.lower()}'] = df_final[df_final['sport_type'] == sport].groupby('athlete_id')['activity_id'].nunique()

    df_athletes_stats['athlete_id'] = df_final['athlete_id'].unique()
    ...
    fc_valid = df_final[df_final['max_heartrate_activity'] < 210]
    ...
    for dist_km in [5, 10, 21.1, 42.2]:
        col = f"{int(dist_km)}km PB" if dist_km in [5, 10] else f"{int(dist_km)}km PB"
    ...
    df_athletes_stats['Nb_activities'] = df_athletes_stats[[
        'Nb_activities_run', 'Nb_activities_ride', 'Nb_activities_swim', 'Nb_activities_workout']].sum(axis=1)
    ...
    df_athletes_stats['VDOT_max'] = df_athletes_stats[[
        'VDOT_5km', 'VDOT_10km', 'VDOT_21km', 'VDOT_42km']].max(axis=1)
...
def enrich_activities_with_training_load(df_final, df_athletes_stats):
    ...
    df_agg = df_final[['athlete_id', 'activity_id', 'start_date', 'cv_speed', 'pct_Z1', 'pct_Z2', 'pct_Z3', 'pct_Z4', 'sport_type']].copy()
    ...
    df_agg['intensity'] = (df_agg["pct_Z1"] * 1 + df_agg["pct_Z2"] * 1.5 + df_agg["pct_Z3"] * 2.25 + df_agg["pct_Z4"] * 3)
...
if __name__ == "__main__":
    ...
    # === 2. Generate athlete-level stats ===
    df_athletes_stats = generate_athlete_stats(df_final, df_best_efforts, manual_stats_path)

    # === 3. Enrich activity data with splits and training load ===
    df_final_enriched, df_agg, dfs_by_sport = enrich_activities_with_training_load(df_final, df_athletes_stats)

    # === 4. Save results ===
    df_athletes_stats.to_csv(DATA_DIR / "df_athletes_stats.csv", index=False)
    df_final_enriched.to_csv(DATA_DIR / "df_final_enriched.csv", index=False)
    df_agg.to_csv(DATA_DIR / "df_activities_agg.csv", index=False)

    for sport, df_sport in dfs_by_sport.items():
        df_sport.to_csv(DATA_DIR / f"df_activities_{sport.lower()}.csv", index=False)
